...
非ASCIIのUTF-8 NFC正規化により基本的にバイト数が減るため、データは前方（ファイル先頭方向）にずれます。
これにより `writer offset < reader offset` が保たれるため、一時ファイルを作成することなくインプレースで安全に実行できます。

また、空いたバイトを可能な限り extra field（パディング）で埋めることで、後続のデータを動かさないようにし、ディスク書き込みを最小限に抑えています。
